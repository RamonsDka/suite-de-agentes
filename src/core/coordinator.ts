...
import type { AgentPermissions, CoordinatorConfig, InterviewCheckpoint, InterviewTranscript, InterviewTurn } from "./types.ts";
...
/** @deprecated Kept until the interview UI replaces every one-shot caller. */
...
const LEGACY_DRAFT_KEYS = ["id", "description", "systemPrompt", "operations", "model", "effort", "skills", "permissions"];
const LEGACY_PERMISSION_VALUES = new Set(["allow", "ask", "deny"]);
...
export function parseAgentDraft(response: string): AgentDraft {
  ...
  if (Object.keys(draft).length !== LEGACY_DRAFT_KEYS.length || Object.keys(draft).some((key) => !LEGACY_DRAFT_KEYS.includes(key))) throw new Error("Coordinator draft must use the strict draft shape.");
  ...
  if (!permissions || typeof permissions !== "object" || Array.isArray(permissions) || Object.keys(permissions).some((key) => !key || ["__proto__", "constructor", "prototype"].includes(key) || !LEGACY_PERMISSION_VALUES.has((permissions as Record<string, unknown>)[key] as string))) throw new Error("Coordinator draft has invalid permissions.");
