...
The system MUST allow users to describe agent intent in plain language and use the AI coordinator to generate or refine structured agent fields including `name`, `description`, `systemPrompt`, and `operations`.

**User Story:** As a creator, I want to describe intent in natural language so that the coordinator creates prompt instructions and capabilities.

#### Acceptance & Edge Case Checklist
- [ ] Natural language intent generates candidate agent fields.
- [ ] Conversational feedback refines targeted fields iteratively.
...
- GIVEN a user enters a description of desired agent behavior
- WHEN the coordinator processes the prompt
- THEN a candidate agent definition with description and operations is generated

#### Scenario: Refine agent from conversational feedback
- GIVEN an existing agent definition is loaded in authoring
- WHEN the user submits refinement feedback
...
The system MUST NOT apply generated or refined agent attributes automatically. All proposed additions, modifications, or deletions MUST be presented in a structured preview offering three explicit actions: `Approve`, `Request changes`, and `Discard`.

**User Story:** As a creator, I want to inspect AI-generated changes in a structured preview so that I retain full control before applying edits.
