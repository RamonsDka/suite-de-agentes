...
The system MUST gate AI-assisted actions behind coordinator configuration. If an unconfigured action is invoked, the system MUST display `Configurar ahora` and `Cancelar`. Selecting `Cancelar` MUST return to the original user intent without loss. AI tasks MUST execute in tool-less ephemeral sessions with cancellable foreground progress.

**User Story:** As a user, I want unconfigured AI actions gated with clear guidance so that I can configure the coordinator or cancel safely.
